...
class _PatientsScreenState extends ConsumerState<PatientsScreen> {
                    ...
                    children: [
                      ...
                      Expanded(
                        child: patientsState.when(
                          ...
                          error: (error) =>
                              _buildEmptyState(error, isError: true),
                          data: (patients) => patients.isEmpty
                              ? _buildEmptyState('', isError: false)
                              : _buildPatientsList(patients, isNarrow),
                        ),
                      ),
                    ],
                  ),
              ...
              Container(
                ...
                decoration: const BoxDecoration(
                  gradient: LinearGradient(
                    colors: [_kHeaderGradient1, _kHeaderGradient2],
                  ),
                  border: Border(top: BorderSide(color: _kGold, width: 2)),
                ),
                child: Text(
                  'Doctor Scribe AI — פרטי קשר | תמיכה | הגדרות',
  ...
  Widget _buildPatientsList(List<PatientModel> patients, bool isNarrow) {
    return ListView.separated(
      ...
      itemBuilder: (context, index) =>
          _buildPatientCard(patients[index], isNarrow),
    );
  }
          ...
          child: Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: const LinearGradient(
                    colors: [Color(0xFFFFFFFF), Color(0xFFC8D8FF)],
                  ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (patient.phone != null && patient.phone!.isNotEmpty)
                      Text(
                        patient.phone!,
                        style: GoogleFonts.rubik(fontSize: 12, color: _kMuted),
                      ),
                  ],
                ),
              ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            isError ? 'patients.load_error'.tr() : 'patients.empty'.tr(),
            style: GoogleFonts.rubik(
              fontSize: 16,
              fontWeight: FontWeight.w600,
              color: _kNavy,
          ...
          Text(
            isError ? '' : 'patients.empty_hint'.tr(),
            style: GoogleFonts.rubik(fontSize: 13, color: _kMuted),
          ),
          if (isError) ...[
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'המטופל ${patient.name} נמחק בהצלחה',
              style: GoogleFonts.heebo(),
            ),
  ...
  Future<void> _exportCsv() async {
    ...
    if (patients.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('אין מטופלים לייצוא', style: GoogleFonts.heebo()),
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('הקובץ הורד בהצלחה', style: GoogleFonts.heebo()),
            backgroundColor: Colors.green.shade700,
          ),
        );
    } catch (_) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('שגיאה בייצוא', style: GoogleFonts.heebo()),
            backgroundColor: Colors.red.shade700,
          ),
        );
    }
  ...
  Future<void> _importCsv() async {
    try {
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'קובץ: ${file.name}',
                style: GoogleFonts.heebo(fontSize: 14, color: Colors.white70),
              ),
              const SizedBox(height: 10),
              Text(
                'מטופלים קיימים לא יידרסו — רק חדשים ייווצרו.',
                style: GoogleFonts.heebo(
                  fontSize: 13,
          ...
          actions: [
            TextButton(
              ...
              child: Text(
                'ביטול',
                style: GoogleFonts.heebo(color: Colors.white60),
              ),
            ),
            ElevatedButton(
      ...
      if (mounted) {
        final msg =
            'יובאו ${data['created_patients']} מטופלים ו-${data['created_visits']} ביקורים';
        final errCount = (data['errors'] as List?)?.length ?? 0;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              errCount > 0 ? '$msg ($errCount שגיאות)' : msg,
              style: GoogleFonts.heebo(),
            ),
  ...
  Future<void> _downloadSampleCsv() async {
    try {
      final dio = ApiClient().dio;
      final response = await dio.get(
        '/patients/import/sample-csv',
        options: Options(responseType: ResponseType.bytes),
      );
      final blob = html.Blob([response.data], 'text/csv');
      final url = html.Url.createObjectUrlFromBlob(blob);
      html.AnchorElement(href: url)
        ..setAttribute('download', 'import_template.csv')
        ..click();
      html.Url.revokeObjectUrl(url);
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _importHandwritten() async {
    try {
      final result = await FilePicker.platform.pickFiles(
        type: FileType.custom,
      ...
      final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                '${validFiles.length} תמונות נבחרו:',
                style: GoogleFonts.heebo(fontSize: 14, color: Colors.white70),
              ),
              ...
              Container(
                ...
                child: Text(
                  'הבינה המלאכותית תנתח את התמונות ותחלץ פרטי מטופל וסיכום רפואי אוטומטית.',
                  style: GoogleFonts.heebo(
                    fontSize: 12,
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Text('מנתח תמונות...', style: GoogleFonts.heebo()),
              ],
            ),
      ...
      if (mounted) {
        ...
        final patientName = data['patient']?['name'] ?? 'מטופל';
        final visits = data['created_visits'] ?? 0;
        final msg =
            'המטופל "$patientName" יובא בהצלחה${visits > 0 ? " כולל סיכום רפואי" : ""}';
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(msg, style: GoogleFonts.heebo()),
            backgroundColor: Colors.green.shade700,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).hideCurrentSnackBar();
        final msg = e.response?.data?['message'] ?? 'שגיאה בעיבוד התמונות';
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(msg, style: GoogleFonts.heebo()),
            backgroundColor: Colors.red.shade700,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('שגיאה בייבוא מכתב יד', style: GoogleFonts.heebo()),
            backgroundColor: Colors.red.shade700,
          ),
        );
      }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              'בחר מטופלים לייצוא כולל היסטוריית ביקורים',
              style: GoogleFonts.rubik(fontSize: 13, color: _kMuted),
            ),
            const SizedBox(height: 14),
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      'כל המטופלים (${widget.patients.length})',
                      style: GoogleFonts.rubik(
                        fontSize: 14,
                        fontWeight: FontWeight.w600,
                        color: _kNavy,
            ...
            Flexible(
              child: Container(
                decoration: BoxDecoration(
                  border: Border.all(
                    color: _kInputBorder.withValues(alpha: 0.2),
                  ),
                  borderRadius: BorderRadius.circular(10),
                ),
                    ...
                    return InkWell(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          border: Border(
                            bottom: BorderSide(
                              color: _kInputBorder.withValues(alpha: 0.1),
                            ),
                          ),
                          color: isChecked
                        ...
                        child: Row(
                          children: [
                            Checkbox(
                              value: isChecked,
                              onChanged: (_) => _toggle(p.displayId),
                              activeColor: _kNavy,
                              checkColor: Colors.white,
                            ),
                            const SizedBox(width: 6),
            ...
            Row(
              ...
              children: [
                TextButton(
                  ...
                  child: Text(
                    'ביטול',
                    style: GoogleFonts.rubik(color: _kMuted),
                  ),
                ),
                const SizedBox(width: 10),
                ElevatedButton.icon(
                  onPressed: _selected.isEmpty
                      ...
                      : () {
                          Navigator.pop(
                            context,
                            _selectAll ? <int>[] : _selected.toList(),
                          );
                        },
                  ...
                  label: Text(
                    _selected.isEmpty
                        ? 'בחר מטופלים'
                        : 'ייצא ${_selectAll ? "הכל" : "${_selected.length} מטופלים"}',
