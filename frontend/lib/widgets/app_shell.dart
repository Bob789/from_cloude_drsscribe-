          ...
          children: [
            ...
            InkWell(
              ...
              child: Stack(
                ...
                children: [
                  Icon(Icons.mail_outline_rounded, size: 20, color: AppColors.textMuted),
                  if (_unreadCount > 0)
                    Positioned(
                      top: -6,
                      right: -8,
                      child: Container(
                        ...
                        child: Center(
                          child: Text(
                            ...
                            style: GoogleFonts.heebo(fontSize: 9, fontWeight: FontWeight.w700, color: Colors.white),
                          ),
                        ),
                      ),
                    ),
              ...
              child: Stack(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(Icons.mail_outline_rounded, size: 22, color: AppColors.primary),
                  ),
                  if (_unreadCount > 0)
                    Positioned(
                      top: -4,
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: const Color(0xFFEF4444),
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: const Color(0xFF141828), width: 1.5),
                        ),
                        constraints: const BoxConstraints(minWidth: 18, minHeight: 18),
                        child: Center(
                          child: Text(
                            _unreadCount > 99 ? '99+' : '$_unreadCount',
                            style: GoogleFonts.heebo(fontSize: 10, fontWeight: FontWeight.w700, color: Colors.white),
                          ),
                        ),
                      ),
                    ),
...
class _MessagesDialogState extends State<_MessagesDialog> {
  ...
  Future<void> _loadThreads() async {
    try {
      ...
      if (mounted) setState(() { _threads = res.data is List ? res.data : []; _loading = false; });
    } catch (_) {
  ...
  Future<void> _openThread(Map<String, dynamic> thread) async {
    setState(() { _selectedThread = thread; _threadLoading = true; _showCompose = false; _replyCtrl.clear(); });
    try {
      final dio = ApiClient().dio;
      final res = await dio.get('/messages/thread/${thread['thread_id']}');
      if (mounted) setState(() { _threadMessages = res.data is List ? res.data : []; _threadLoading = false; });
    } catch (_) {
  ...
  Future<void> _pickFile() async {
    try {
      final result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: ['jpg', 'jpeg', 'png', 'gif', 'webp', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'csv', 'txt', 'rtf'],
        withData: true,
      );
      ...
      if (file.size > 10 * 1024 * 1024) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('קובץ גדול מדי — מקסימום 10MB', style: GoogleFonts.heebo()), backgroundColor: Colors.orange.shade700));
        return;
      }
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg, style: GoogleFonts.heebo()), backgroundColor: Colors.red.shade700));
    } catch (_) {
      setState(() => _uploading = false);
    }
  }
  ...
  Future<void> _sendNewMessage() async {
    ...
    try {
      ...
      if (mounted) {
        _subjectCtrl.clear();
        _bodyCtrl.clear();
        setState(() { _category = 'general'; _sending = false; _showCompose = false; _attachments = []; });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('ההודעה נשלחה בהצלחה', style: GoogleFonts.heebo()), backgroundColor: Colors.green.shade700));
        _loadThreads();
      }
    } on DioException catch (_) {
      setState(() => _sending = false);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('שגיאה בשליחת ההודעה', style: GoogleFonts.heebo()), backgroundColor: Colors.red.shade700));
    }
  }
  ...
  InputDecoration _inputDeco(String label, {bool alignTop = false}) => InputDecoration(
    labelText: label,
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: Container(
        ...
        height: dialogH,
        decoration: BoxDecoration(
          gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF141828), Color(0xFF0E1220)]),
          borderRadius: BorderRadius.circular(24),
          border: Border.all(color: AppColors.primary.withValues(alpha: 0.2)),
          boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.5), blurRadius: 40, spreadRadius: 4)],
        ),
        child: Column(
          children: [
            // Header
            Container(
              ...
              child: Row(children: [
                Container(
                  ...
                  decoration: BoxDecoration(gradient: LinearGradient(colors: [AppColors.primary.withValues(alpha: 0.2), AppColors.primaryLight.withValues(alpha: 0.1)]), borderRadius: BorderRadius.circular(14)),
                  child: Icon(Icons.mail_rounded, color: AppColors.primary, size: 22),
                ),
                const SizedBox(width: 14),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('הודעות', style: GoogleFonts.heebo(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.white)),
                  Text('צוות התמיכה · Doctor Scribe AI', style: GoogleFonts.heebo(fontSize: 12, color: AppColors.textMuted)),
                ])),
                // New message button
                IconButton(
                  onPressed: () => setState(() { _showCompose = true; _selectedThread = null; }),
                  tooltip: 'הודעה חדשה',
                  icon: Icon(Icons.edit_rounded, color: AppColors.primary, size: 20),
                ),
                IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close_rounded, color: Colors.white38, size: 22)),
              ]),
            ),
            ...
            Expanded(
              child: Row(children: [
                ...
                Container(
                  width: dialogW * 0.38,
                  decoration: BoxDecoration(border: Border(left: BorderSide(color: Colors.white.withValues(alpha: 0.06)))),
                  child: _loading
                      ? const Center(child: CircularProgressIndicator())
                      : _threads.isEmpty
                          ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                              Icon(Icons.inbox_rounded, size: 40, color: AppColors.textMuted.withValues(alpha: 0.3)),
                              const SizedBox(height: 8),
                              Text('אין שיחות', style: GoogleFonts.heebo(fontSize: 14, color: AppColors.textMuted)),
                            ]))
                          : ListView.builder(
                              itemCount: _threads.length,
                              itemBuilder: (ctx, i) {
                                final t = _threads[i] as Map<String, dynamic>;
                                final selected = _selectedThread?['thread_id'] == t['thread_id'];
                                  ...
                                  child: Container(
                                    ...
                                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Row(children: [
                                        ...
                                        Expanded(child: Text(t['subject'] ?? '', style: GoogleFonts.heebo(fontSize: 13, fontWeight: unread > 0 ? FontWeight.w700 : FontWeight.w400, color: Colors.white), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                      ]),
                                      const SizedBox(height: 2),
                                      Text(
                                        t['last_message']?['body']?.toString().replaceAll('\n', ' ') ?? '',
                                        style: GoogleFonts.heebo(fontSize: 12, color: AppColors.textMuted),
                                        maxLines: 1, overflow: TextOverflow.ellipsis,
                                      ),
                                      const SizedBox(height: 2),
                                      Row(children: [
                                        Text(t['last_activity']?.toString().substring(0, 10) ?? '', style: GoogleFonts.heebo(fontSize: 10, color: AppColors.textMuted)),
                                        const Spacer(),
                                        if ((t['message_count'] ?? 0) > 1) Text('${t['message_count']}', style: GoogleFonts.heebo(fontSize: 10, color: AppColors.textMuted)),
                                      ]),
                                    ]),
                                  ),
                                );
                ...
                Expanded(child: _showCompose ? _buildComposePanel() : _selectedThread != null ? _buildThreadPanel() : _buildEmptyState()),
              ]),
            ),
          ],
        ),
  ...
  Widget _buildEmptyState() => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      Icon(Icons.forum_rounded, size: 48, color: AppColors.textMuted.withValues(alpha: 0.3)),
      const SizedBox(height: 12),
      Text('בחר שיחה או צור הודעה חדשה', style: GoogleFonts.heebo(fontSize: 14, color: AppColors.textMuted)),
    ]),
  );
  ...
  Widget _buildComposePanel() => Padding(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('הודעה חדשה לצוות התמיכה', style: GoogleFonts.heebo(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white)),
      const SizedBox(height: 16),
      DropdownButtonFormField<String>(
        ...
        items: [
          DropdownMenuItem(value: 'general', child: Text('כללי', style: GoogleFonts.heebo())),
          DropdownMenuItem(value: 'bug', child: Text('דיווח על תקלה', style: GoogleFonts.heebo())),
          DropdownMenuItem(value: 'feature', child: Text('בקשת תכונה', style: GoogleFonts.heebo())),
          DropdownMenuItem(value: 'improvement', child: Text('הצעה לשיפור', style: GoogleFonts.heebo())),
          DropdownMenuItem(value: 'billing', child: Text('חיוב ותשלום', style: GoogleFonts.heebo())),
        ],
        onChanged: (v) => setState(() => _category = v ?? 'general'),
      ),
      const SizedBox(height: 12),
      TextField(controller: _subjectCtrl, style: GoogleFonts.heebo(color: Colors.white, fontSize: 14), decoration: _inputDeco('נושא')),
      const SizedBox(height: 12),
      Expanded(child: TextField(controller: _bodyCtrl, maxLines: null, expands: true, textAlignVertical: TextAlignVertical.top, style: GoogleFonts.heebo(color: Colors.white, fontSize: 14), decoration: _inputDeco('תוכן ההודעה', alignTop: true))),
      ...
      if (_attachments.isNotEmpty)
        Wrap(
          ...
          children: _attachments.map((a) => Container(
            ...
            decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.primary.withValues(alpha: 0.2))),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.attach_file_rounded, size: 14, color: AppColors.primary),
              const SizedBox(width: 4),
              Text(a['name'] ?? '', style: GoogleFonts.heebo(fontSize: 12, color: AppColors.primary)),
              const SizedBox(width: 6),
              InkWell(
      ...
      Row(children: [
        ...
        IconButton(
          ...
          icon: _uploading
              ...
              : Icon(Icons.attach_file_rounded, size: 20, color: AppColors.textMuted),
        ),
        const SizedBox(width: 4),
        Expanded(
          child: SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_sending ? 'שולח...' : 'שלח הודעה', style: GoogleFonts.heebo(fontSize: 14, fontWeight: FontWeight.w700)),
              style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
            ),
          ),
        ),
        const SizedBox(width: 10),
        TextButton(onPressed: () => setState(() { _showCompose = false; _attachments = []; }), child: Text('ביטול', style: GoogleFonts.heebo(color: AppColors.textMuted))),
      ]),
    ]),
  );
  ...
  Widget _buildThreadPanel() {
    return Column(children: [
      ...
      Container(
        ...
        child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(_selectedThread!['subject'] ?? '', style: GoogleFonts.heebo(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
            Text('${_selectedThread!['message_count'] ?? 0} הודעות', style: GoogleFonts.heebo(fontSize: 11, color: AppColors.textMuted)),
          ])),
        ]),
      ),
      // Messages — chat-style
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return Align(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isOutbound ? AppColors.primary.withValues(alpha: 0.1) : Colors.white.withValues(alpha: 0.05),
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(color: isOutbound ? AppColors.primary.withValues(alpha: 0.2) : Colors.white.withValues(alpha: 0.06)),
                      ),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          Icon(isOutbound ? Icons.support_agent_rounded : Icons.person_rounded, size: 14, color: isOutbound ? AppColors.primary : AppColors.textMuted),
                          const SizedBox(width: 6),
                          Text(isOutbound ? 'צוות תמיכה' : 'את/ה', style: GoogleFonts.heebo(fontSize: 11, fontWeight: FontWeight.w600, color: isOutbound ? AppColors.primary : AppColors.textMuted)),
                          const Spacer(),
                          Text(m['created_at']?.toString().substring(0, 16).replaceAll('T', ' ') ?? '', style: GoogleFonts.heebo(fontSize: 10, color: AppColors.textMuted)),
                        ]),
                        const SizedBox(height: 6),
                        Text(m['body'] ?? '', style: GoogleFonts.heebo(fontSize: 13, color: AppColors.textSecondary, height: 1.6)),
                      ]),
                    ),
                  );
                },
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'כתוב תגובה...',
                hintStyle: GoogleFonts.heebo(color: Colors.white24, fontSize: 13),
                filled: true,
                fillColor: Colors.white.withValues(alpha: 0.04),
