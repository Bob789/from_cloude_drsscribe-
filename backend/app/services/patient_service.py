...
async def search_patients(db: AsyncSession, query: str, user_id: uuid.UUID | None = None) -> list[Patient]:
    ...
    # All PII fields are encrypted — SQL ILIKE cannot match plaintext.
    # Load all patients and filter in Python after decryption.
    all_result = await db.execute(select(Patient).where(owner_filter))
    all_patients = all_result.scalars().all()
    all_prepared = [prepare_patient(db, p) for p in all_patients]
    ...
    for p in all_prepared:
        ...
        if p.name and query_lower in p.name.lower():
            ...
            continue
        # Email search
        if p.email and query_lower in p.email.lower():
            matches[p.id] = p
            continue
        # Phone search (digits only)
        if len(digits) >= 3 and p.phone:
            phone_digits = ''.join(c for c in p.phone if c.isdigit())
            if digits in phone_digits:
                matches[p.id] = p
                continue
        # ID number search
        if len(digits) >= 3 and p.id_number:
            if digits in p.id_number:
                matches[p.id] = p
                continue
