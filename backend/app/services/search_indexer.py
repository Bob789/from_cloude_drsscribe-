...
from app.services.search_service import get_search_client, _get_tags_for_summary
...
async def init_indexes():
    ...
    client.index("transcriptions").update_settings({
        ...
        "filterableAttributes": ["recording_id", "language", "status", "created_at", "tags", "visit_id", "doctor_id"],
    ...
    client.index("summaries").update_settings({
        ...
        "filterableAttributes": ["visit_id", "urgency", "status", "created_at", "tags", "doctor_id"],
        "sortableAttributes": ["created_at"],
    })
...
async def reindex_all(db: AsyncSession):
    ...
    for s in summaries:
        ...
        visit_r = await db.execute(select(Visit).where(Visit.id == s.visit_id))
    ...
    for t in transcriptions:
        ...
        patient_name, patient_display_id, visit_id, doctor_id = "", 0, "", ""
        if rec:
...
async def reindex_summary(db: AsyncSession, summary: Summary):
    visit_r = await db.execute(select(Visit).where(Visit.id == summary.visit_id))
    visit = visit_r.scalar_one_or_none()
    doctor_id = str(visit.doctor_id) if visit else ""

    pat_r = await db.execute(select(Patient).where(Patient.id == visit.patient_id)) if visit else None
    pat = pat_r.scalar_one_or_none() if pat_r else None
    patient_name, patient_display_id = "", 0
    if pat:
        db.expunge(pat)
        decrypt_patient_pii(pat)
    ...
    # Decrypt summary fields for indexing
    db.expunge(summary)
    decrypt_summary_fields(summary)

    await index_summary(str(summary.id), {
        "visit_id": str(summary.visit_id), "doctor_id": doctor_id,
        "patient_name": patient_name, "patient_display_id": patient_display_id,
