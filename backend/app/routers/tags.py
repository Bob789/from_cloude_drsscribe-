...
async def list_tags(
    ...
    result = await db.execute(
        ...
        .join(Summary, (Tag.entity_type == "summary") & (Tag.entity_id == cast(Summary.id, String)))
...
async def visits_by_tag(
    ...
    result = await db.execute(
        ...
        .join(Summary, (Tag.entity_type == "summary") & (Tag.entity_id == cast(Summary.id, String)))
        .join(Visit, Summary.visit_id == Visit.id)
        .where(Tag.tag_code == tag_code, Visit.doctor_id == current_user.id)
        .order_by(Tag.created_at.desc())
    )
    tags = result.scalars().all()
    visit_ids = [t.entity_id for t in tags]
