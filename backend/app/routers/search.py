...
async def search_all(
    ...
    if not q and tag_list:
        result = await search_by_tags(db, tag_list, date_from, date_to, offset, per_page, doctor_id=current_user.id)
    ...
    try:
        result = await combined_search(q, tag_list, date_from, date_to, offset, per_page, doctor_id=doctor_id)
        if result["total"] > 0:
            return SearchResult(hits=result["hits"], total=result["total"], page=page, per_page=per_page)
    except Exception:
        pass
    ...
    result = await search_db_text(db, q, tag_list, date_from, date_to, offset, per_page, doctor_id=current_user.id)
    return SearchResult(hits=result["hits"], total=result["total"], page=page, per_page=per_page)
